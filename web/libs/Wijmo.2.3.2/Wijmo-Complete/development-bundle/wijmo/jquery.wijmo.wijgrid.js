	...
	$.widget("wijmo.wijgrid", {
		...
		_fieldResized: function (fieldWidget, oldWidth, newWidth) {
			...
			if (this._trigger("columnResizing", null, { column: fieldWidget.options, oldWidth: oldWidth, newWidth: newWidth }) !== false) {
				var column = $.wijmo.wijgrid.getColumnByTravIdx(this.options.columns, fieldWidget.options.travIdx);
				if (column) {
					column.found.width = newWidth; // update options
				}

				fieldWidget.option("width", newWidth); // update widget
